...
es_out_t *input_EsOutNew( input_thread_t *p_input, int i_rate )
{
    ...
    p_sys->buffering_percent = -1;
    p_input->buf_param = &p_sys->buffering_percent;
...
static void EsOutDecodersStopBuffering( es_out_t *out, bool b_forced )
{
    ...
    if( i_stream_duration <= i_buffering_duration && !b_forced )
    {
	    ...
	    p_sys->buffering_percent = (100* f_level);
        //msg_Dbg( p_sys->p_input, "MY PERCENT %d", (int) p_sys->buffering_percent);
        return;
    }
    else if(p_sys->buffering_percent < 100){
        p_sys->buffering_percent = 100;
        return;
    }
